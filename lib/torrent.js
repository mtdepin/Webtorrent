...
class Torrent extends EventEmitter {
  ...
  _onMetadata (metadata) {
    ...
    if (this._pieceDownload || this._pieceSeed) {
      ...
      const lastPieceLength = (this._pieceEnd === this.pieces.length - 1) ? this.lastPieceLength : this.pieceLength
      if (this._pieceDownload) {
        const fname = 'pieces-' + this._pieceStart + '-' + this._pieceEnd
        piecesOffset = this._pieceStart * this.pieceLength
        this.files = [{
          path: fname,
          name: fname,
          length: (this._pieceEnd - this._pieceStart) * this.pieceLength + lastPieceLength,
          offset: piecesOffset
        }]
        ...
        piecesOffset = this._pieceStart * this.pieceLength
        this.files = [{
          path: this._pieceSeedPath,
          name: this._pieceSeedPath,
          length: (this._pieceEnd - this._pieceStart) * this.pieceLength + lastPieceLength,
          offset: piecesOffset
        }]
    ...
    if (this.skipVerify) {
      ...
      if (this._pieceSeed) {
        this._markPiecesVerified(this._pieceStart, this._pieceEnd)
      } else {
