...
class Torrent extends EventEmitter {
  constructor (torrentId, client, opts) {
    ...
    this.announceAuth = opts.announceAuth
    ...
    this._pieceDownload = opts.pieceDownload || false
    this._pieceSeed = opts.pieceSeed || false
    this._pieceStart = opts.pieceStart
    this._pieceEnd = opts.pieceEnd
    this._pieceRange = opts.pieceRange || []
    this._pieceSeedPath = opts.pieceSeedPath || null
    this._mdonly = opts.mdonly || false
    this._fixPath = opts.fixPath || false

    if ((this._pieceRange.length === 0) && (typeof opts.pieceStart === 'number') && (typeof opts.pieceEnd === 'number')) {
      this._pieceRange.push({
        pieceStart: this._pieceStart,
        pieceEnd: this._pieceEnd,
        done: false
      })
    }
  ...
  get progress () { return this.downloadLength ? this.downloaded / this.downloadLength : 0 }
  ...
  _onTorrentId (torrentId) {
    ...
    if (parsedTorrent) {
      ...
      //console.log('_onTorrentId:', this._debugId)
      queueMicrotask(() => {
        if (this.destroyed) return
        this._onParsedTorrent(parsedTorrent)
      })
    } else {
      //console.log('_onTorrentId: failed parse torrent')
  ...
  _onParsedTorrent (parsedTorrent) {
    ...
    //console.log('_onParsedTorrent: _processParsedTorrent')
    ...
    //console.log('_onParsedTorrent: emit infohash')
    ...
    if (trackerOpts) {
      trackerOpts = Object.assign({}, this.client.tracker, {
        getAnnounceOpts: () => {
          ...
          //let have_pieces, want_pieces
          //console.log('_startDiscovery: getAnnounceOpts', this._pieceDownload, this._pieceSeed, this._pieceRange)
          //if (this._pieceDownload || this._pieceSeed) {
          //  this._pieceRange.forEach(range => {
          //    if (range.done) {
          //      if (typeof have_pieces === 'undefined') have_pieces = []
          //      have_pieces.push([range.pieceStart, range.pieceEnd])
          //   } else {
          //      if (typeof want_pieces === 'undefined') want_pieces = []
          //      want_pieces.push([range.pieceStart, range.pieceEnd])
          //    }
          //  })
          //}
  ...
  _onMetadata (metadata) {
    ...
    if (this._pieceDownload || this._pieceSeed) {
      //console.log('_onMetadata: piece download/seed', this.files)
      if (this._pieceDownload) {
        this.files = []
        this._pieceRange.forEach(range => {
          const start = range.pieceStart
          const end = range.pieceEnd
          const fname = 'pieces-' + start + '-' + end
          const lastPieceLength = (end === this.pieces.length - 1) ? this.lastPieceLength : this.pieceLength
          const fileLength = (end - start) * this.pieceLength + lastPieceLength,
          piecesOffset = start * this.pieceLength
          this.files.push({
            path: fname,
            name: fname,
            length: fileLength,
        ...
        console.log('_onMetadata: piece download', this.files, this.pieces.length, this.lastPieceLength, this.pieceLength)
      } else {
        const seedPaths = [].concat(this._pieceSeedPath)
        if (seedPaths.length !== this._pieceRange.length) {
          console.log('_onMetadata: Bad piece seed', seedPaths, this._pieceRange)
          this._destroy(new Error('Bad piece seed'))
          return
        }
        this.files = []
        for (let i = 0; i < seedPaths.length; i++) {
          const start = this._pieceRange[i].pieceStart
          const end = this._pieceRange[i].pieceEnd
          const lastPieceLength = (end === this.pieces.length - 1) ? this.lastPieceLength : this.pieceLength
          piecesOffset = start * this.pieceLength
          this.files.push({
            path: seedPaths[i],
            name: seedPaths[i],
            length: (end - start) * this.pieceLength + lastPieceLength,
            offset: piecesOffset
          })
        }
        console.log('_onMetadata: piece seed', this.files)
      }
    }

    if (this._fixPath) {
      let path = `${this.path}/${this.infoHash}`
      this.path = path
    }

    this.downloadLength = downloadLength > 0 ? downloadLength : this.length
    ...
    if (!rawStore) {
      rawStore = new this._store(this.pieceLength, {
        ...
        files: this.files,
        length: this.length,
        pieceFiles: (this._pieceDownload || this._pieceSeed),
    ...
    this.pieces = this.pieces.map((hash, i) => {
      const pieceLength = (i === this.pieces.length - 1)
        ...
        : this.pieceLength
      //console.log('_onMetadata: new Piece', i)
    ...
    if (this._mdonly) return

    if (this.skipVerify) {
      // Skip verifying exisitng data and just assume it's correct
      if (this._pieceSeed) {
        this._pieceRange.forEach(range => {
          this._markPiecesVerified(range.pieceStart, range.pieceEnd)
        })
      } else {
  ...
  addPiece (pieceStart, pieceEnd, path) {
    if (this.destroyed || (!this.metadata)) return
    let result = this._pieceRange.find(v => {
      return Math.max(v.pieceStart, pieceStart) <= Math.min(v.pieceEnd, pieceEnd)
    })
    if (result) {
      const err = new Error('duplication piece range')
    ...
    const fname = path || 'pieces-' + pieceStart + '-' + pieceEnd
    const piecesOffset = pieceStart * this.pieceLength
    const lastPieceLength = (pieceEnd === this.pieces.length - 1) ? this.lastPieceLength : this.pieceLength
    const fileLength = (pieceEnd - pieceStart) * this.pieceLength + lastPieceLength
    ...
    this.store.add(file)
    file.select()
    if (this.downloadLength) this.downloadLength += fileLength
    else this.downloadLength = fileLength
    console.log('addPiece: torrent', this._pieceRange, this._pieceSeedPath)
    return true
  }
  ...
  _onWire (wire, addr) {
    ...
    //console.log('got wire', wire._debugId, addr)
  ...
  _onWireWithMetadata (wire) {
    ...
    const updateSeedStatus = () => {
      ...
      //console.log('updateSeedStatus: wire is seeder, choke it', i)
    }

    wire.on('bitfield', () => {
      //console.log('_onWireWithMetadata: on bitfield')
      updateSeedStatus()
      this._update()
    ...
    wire.on('have', () => {
      //console.log('_onWireWithMetadata: on have')
      updateSeedStatus()
      this._update()
      this._updateWireInterest(wire)
    })
    ...
    wire.lt_donthave.on('donthave', () => {
      //console.log('_onWireWithMetadata: on donthave')
      updateSeedStatus()
      this._update()
      this._updateWireInterest(wire)
    })
    ...
    // fast extension (BEP6)
    wire.on('have-all', () => {
      //console.log('_onWireWithMetadata: on have all, wire is seeder')
      wire.isSeeder = true
      wire.choke() // always choke seeders
      this._update()
      this._updateWireInterest(wire)
    ...
    // fast extension (BEP6)
    wire.on('have-none', () => {
      //console.log('_onWireWithMetadata: on have none, wire isnot seeder')
      wire.isSeeder = false
      this._update()
      this._updateWireInterest(wire)
    })
    ...
    wire.once('interested', () => {
      //console.log('_onWireWithMetadata: once interested')
      wire.unchoke()
    })

    wire.once('close', () => {
      //console.log('_onWireWithMetadata: once close')
      clearTimeout(timeoutId)
    })

    wire.on('choke', () => {
      //console.log('_onWireWithMetadata: on choke')
      clearTimeout(timeoutId)
      timeoutId = setTimeout(onChokeTimeout, CHOKE_TIMEOUT)
      if (timeoutId.unref) timeoutId.unref()
    })
    ...
    wire.on('unchoke', () => {
      //console.log('_onWireWithMetadata: on unchoke')
      clearTimeout(timeoutId)
      this._update()
    })

    wire.on('request', (index, offset, length, cb) => {
      //console.log('_onWireWithMetadata: on request', index, offset, length)
  ...
  _updateWireInterest (wire) {
    ...
    //let bitfield = []
    for (let index = 0; index < this.pieces.length; ++index) {
      //bitfield.push(wire.peerPieces.get(index))
      if (this.pieces[index] && wire.peerPieces.get(index)) {
        interested = true
        break
      }
    }
    //console.log('_updateWireInterest:', this.pieces, bitfield, wire.remoteAddress, interested)
  ...
  _request (wire, index, hotswap) {
    ...
    //console.log('wire.request:', index, chunkOffset, chunkLength)
    ...
    wire.request(index, chunkOffset, chunkLength, function onChunk (err, chunk) {
      ...
      if (piece !== self.pieces[index]) {
        //console.log('onChunk: piece !== self.pieces[index]')
        return onUpdateTick()
      }
      ...
      if (err) {
        ...
        //console.log('error getting piece', index, chunkOffset, chunkLength, wire.remoteAddress, err.message)
      ...
      //console.log('got piece', index, chunkOffset, chunkLength, wire.remoteAddress)
  ...
  _checkDone () {
    ...
    this._pieceRange.forEach(range => {
      ...
      for (let i = range.pieceStart; i <= range.pieceEnd; ++i) {
        if (!this.bitfield.get(i)) return
      }
      range.done = true
      this._debug(`pieceRange done: ${range.pieceStart} ${range.pieceEnd}`)
    })
