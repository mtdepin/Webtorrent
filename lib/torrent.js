...
log4js.configure({
  appenders: {
    WebTorrent: { type: 'file', filename: 'webtorrent.log' },
  },
  categories: {
    default: { appenders: ['WebTorrent'], level: 'debug' },
  },
})
...
class Torrent extends EventEmitter {
  ...
  logWire(wire, msg) {
    logger.info('wire:', msg, wire._debugId, wire.type, wire.remoteAddress, wire.remotePort,
      'upload/download', wire.uploaded, wire.downloaded, wire.uploadSpeed(), wire.downloadSpeed())
  }
  ...
  _onWire (wire, addr) {
    ...
    logger.info('got wire', wire._debugId, wire.remoteAddress, wire.remotePort)
  ...
  _onWireWithMetadata (wire) {
    ...
    const onChokeTimeout = () => {
      ...
      if (this._numQueued > 2 * (this._numConns - this.numPeers) &&
        wire.amInterested) {
        this.logWire(wire, 'onChokeTimeout')
        wire.destroy()
      } else {
    ...
    wire.on('request', (index, offset, length, cb) => {
      ...
      if (length > MAX_BLOCK_LENGTH) {
        ...
        this.logWire(wire, 'MAX_BLOCK_LENGTH')
        return wire.destroy()
      }
