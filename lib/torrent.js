...
class Torrent extends EventEmitter {
  constructor (torrentId, client, opts) {
    ...
    this._pieceStart = opts.pieceStart || 0
    this._pieceEnd = opts.pieceEnd || 0
    ...
    if ((typeof opts.pieceStart === 'number') && (typeof opts.pieceEnd === 'number')) {
  ...
  _onMetadata (metadata) {
    ...
    if (this._pieceDownload || this._pieceSeed) {
      ...
      if (this._pieceDownload) {
        ...
        console.log('_onMetadata: piece download', this.files)
      } else {
  ...
  addPiece (pieceStart, pieceEnd, path) {
    ...
    if (result) {
      this._destroy(new Error('duplication piece range'))
      return
    }
